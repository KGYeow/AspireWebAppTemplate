...
namespace AspireWebAppTemplate.Infrastructure.Services.Notifications;
    ...
    public async Task CreateNotificationAsync(CreateNotificationRequest request)
    {
        try
        {
            ...
            try
            {
                ...
                await _webCallbackClient.NotifyAsync(new Application.Features.Notifications.NotificationPushRequest
                {
                    UserId = request.UserId,
                    Title = request.Title,
                    Message = request.Message,

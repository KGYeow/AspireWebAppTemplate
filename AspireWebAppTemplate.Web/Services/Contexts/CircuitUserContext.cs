...
/// and client IP address for the lifetime of a Blazor Server circuit. This solves the
/// fundamental issue where <see cref="IHttpContextAccessor.HttpContext"/> becomes null
/// after the SignalR WebSocket connection replaces the initial HTTP request.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <b>Solution:</b> This scoped service captures the user's <see cref="ClaimsPrincipal"/> and
/// the client's remote IP address during the initial render (when <c>HttpContext</c> is still
/// available) and retains them for the circuit's lifetime. The <see cref="UserIdentityDelegatingHandler"/>
/// reads from this cache first, falling back to <c>HttpContext</c> only during the initial SSR pass.
/// </para>
/// </remarks>
public sealed class CircuitUserContext
{
    ...
    /// <summary>
    /// Gets the cached client IP address for the current circuit.
    /// Returns null if <see cref="Initialize"/> has not been called yet or if
    /// the remote IP address was not available during initialization.
    /// </summary>
    ...
    /// <summary>
    /// Captures the user's claims principal and client IP address for the duration of this circuit.
    ...
    /// <param name="clientIpAddress">The client's remote IP address from HttpContext.Connection.RemoteIpAddress.</param>
    public void Initialize(ClaimsPrincipal user, string? clientIpAddress = null)
    {

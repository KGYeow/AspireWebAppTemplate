...
<!-- TEMPLATE-OWNED steering (Architectural patterns & key design decisions). Inherited from AspireWebAppTemplate.
...
- **Thin Controller / Full Service Layer**: Controllers handle ONLY HTTP concerns (request parsing, status code mapping). All business logic, database access, audit logging, and entity mapping lives in service classes under `Infrastructure/Services/{Feature}/` (feature-first; e.g., `Services/AuditLog/`).
